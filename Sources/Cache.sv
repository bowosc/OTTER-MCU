module Cache(
    ...
    parameter BYTE_OFFSET = 2; //changed from 0 to 2 b/c instructions are 4 bytes
    ...
    logic [2:0] pc_offset;
    logic validity; //prof forgot to add logic for this in the implementation 
    ...
    assign hit = (validity && (cache_tag == pc_tag)); // modify
    assign miss = !hit;
    always_comb begin
        rd = 32'h00000013; //nop
    ...
    always_ff @(negedge CLK) begin
        ...
        data[index][6] <= w6;
        data[index][7] <= w7;
        tags[index] <= pc_tag; //store tag for cache 
